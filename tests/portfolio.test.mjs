...
test('command aliases resolve page paths',()=>{
  assert.deepEqual(parseCommand('cd /projects/'),{name:'projects',args:[]});
  assert.deepEqual(parseCommand('projects systems'),{name:'projects',args:['systems']});
...
test('terminal navigation, keyboard UX, filtering, and untrusted input',async()=>{
  ...
  for(const key of ['window','document','history','location','localStorage','navigator']) Object.defineProperty(globalThis,key,{value:window[key],configurable:true});
  globalThis.matchMedia=window.matchMedia;
  globalThis.requestAnimationFrame=callback=>callback();
  await import('../js/script.js');
  const $=selector=>window.document.querySelector(selector);
  const run=command=>{$('#command-input').value=command;$('#command-form').dispatchEvent(new window.Event('submit',{bubbles:true,cancelable:true}));};
  ...
  assert.equal($('[data-filter="Systems"]').getAttribute('aria-pressed'),'true');
  run('about');
  assert.equal(window.location.pathname,'/about/');
  assert.match($('#terminal-output').textContent,/Mazharuddin/);
  run('contact');
  assert.equal($('a[href^="mailto:"]').getAttribute('href'),'mailto:adnanmazharuddinshaikh@gmail.com');
  ...
  run('theme platinum');assert.equal($('#terminal').dataset.theme,'platinum');
  run('resume');assert.ok($('#terminal-output a[href="/adnanmaz@usc.edu.pdf"]'));
  ...
  run('about');assert.equal($('#terminal-output').innerHTML,contentBeforePowerOff);
  $('#monitor-power').click();
  ...
  $('#command-input').value='hello';$('#command-input').setSelectionRange(2,2);
  $('#command-input').dispatchEvent(new window.Event('input'));
  assert.equal($('#cursor-before').textContent,'he');assert.equal($('#block-cursor').textContent,'l');assert.equal($('#cursor-after').textContent,'lo');
  ...
  commandChip.focus();commandChip.click();
  assert.equal(document.activeElement,$('#command-input'));
  assert.equal(window.location.pathname,'/projects/');
  run('resume');
  ...
  $('#monitor-power').click();$('#monitor-power').click();
  assert.equal($('#terminal').dataset.windowState,'minimized');
  assert.equal($('#command-input').disabled,true);
  ...
  $('#window-minimize').click();$('#restore-terminal').click();
  assert.equal($('#terminal').dataset.windowState,'maximized');
  $('#window-maximize').click();assert.equal($('#terminal').dataset.windowState,'normal');
  $('#window-maximize').click();
  $('#terminal-window').dispatchEvent(new window.KeyboardEvent('keydown',{key:'Escape',bubbles:true}));
  assert.equal($('#terminal').dataset.windowState,'normal');
  $('#window-close').click();
  ...
  $('#window-close').click();$('[data-route="about"]').click();
  assert.equal($('#terminal').dataset.windowState,'normal');
  assert.match($('#terminal-output').textContent,/Mazharuddin/);
  assert.equal($('#motion-toggle'),null);
  assert.equal($('#terminal-expand'),null);
  assert.equal(document.querySelector('.window-dots, .status-dot, .orange-dot, .power-led'),null);
  assert.equal($('#monitor-power svg') !== null,true);
  assert.equal(document.querySelector('.screen-off').textContent,'');
  assert.equal(document.querySelectorAll('.site-footer .social-icon').length,2);
  for(const route of routes){run(route);assert.doesNotMatch($('#terminal-output').textContent,/[\u2014\u2197]/);}
  run('clear');run('contact');assert.equal(document.querySelectorAll('.contact-links .social-icon').length,5);
  run('clear');assert.equal($('#terminal-output').textContent,'');
  $('#command-input').value='proj';
  $('#command-input').dispatchEvent(new window.KeyboardEvent('keydown',{key:'Tab',bubbles:true,cancelable:true}));
  assert.equal($('#command-input').value,'projects');
  $('#command-input').dispatchEvent(new window.KeyboardEvent('keydown',{key:'ArrowUp',bubbles:true,cancelable:true}));
  assert.equal($('#command-input').value,'clear');
  $('#command-input').dispatchEvent(new window.KeyboardEvent('keydown',{key:'ArrowDown',bubbles:true,cancelable:true}));
  assert.equal($('#command-input').value,'projects');
  run('help');assert.match($('#terminal-output').textContent,/Tab completes commands/);
  $('[data-route="home"]').click();assert.equal(window.location.pathname,'/');
  assert.match($('#terminal-output').textContent,/10adnan75/);
  ...
  $('[data-route="about"]').dispatchEvent(modified);assert.equal(modified.defaultPrevented,false);
  window.close();
});
...
test('production routes and original assets are included',async()=>{
  ...
  for(const asset of ['adnanmaz@usc.edu.pdf','img/java.JPG','img/linux.JPG','img/python.JPG','img/avataaars.svg','img/sorting-visualizer-preview.png','img/speed-typing-test-preview.png','img/bug-tracker-preview.png','img/fake-review-preview.jpg','404.html','.nojekyll']) await access(`dist/${asset}`);
  await assert.rejects(access('dist/ams_cv.pdf'));
  const root=await readFile('dist/index.html','utf8');
  assert.doesNotMatch(root,/[\u2014\u2197]/);
  assert.match(root,/property="og:image" content="https:\/\/10adnan75.github.io\/img\/og.png"/);
  assert.match(root,/name="twitter:card" content="summary_large_image"/);
  await access('dist/img/og.png');
  for(const match of root.matchAll(/(?:src|href)="(\/assets\/[^\"]+)"/g)) await access(`dist${match[1]}`);
});
